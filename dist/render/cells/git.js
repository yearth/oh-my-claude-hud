...
registerCell({
    ...
    render(ctx) {
        ...
        const inner = [linkedBranch];
        if (gitConfig?.showAheadBehind) {
            if (ctx.gitStatus.ahead > 0)
                inner.push(formatAheadCount(ctx.gitStatus.ahead, gitConfig, colors));
            if (ctx.gitStatus.behind > 0)
                inner.push(gitBranchColor(`↓${ctx.gitStatus.behind}`, colors));
        }
